...
//   cloud->points.resize (cloud->width * cloud->height);
  ...
  reg.setInputCloud (filtered_transformed_cloud);
  // reg.setIndices (indices);
  reg.setSearchMethod (tree);
  reg.setDistanceThreshold (16);
  reg.setPointColorThreshold (640);
  reg.setRegionColorThreshold (16);
  reg.setMinClusterSize (600);

  std::vector <pcl::PointIndices> clusters;
  reg.extract (clusters);

  pcl::PointCloud <pcl::PointXYZRGB>::Ptr RGS_cloud = reg.getColoredCloud ();
  ...
  viewer->addPointCloud<pcl::PointXYZRGB> (RGS_cloud, "transformed cloud");
